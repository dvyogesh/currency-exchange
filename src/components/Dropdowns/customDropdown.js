...
class customDropdown extends React.Component {
  ...
  render () {
    const {
      classes, value, id, name, menuOptions, handleOnChange, inputLabel, disabled
    } = this.props
    return (
      <div className={classes.formControl}>
        ...
        >
          {menuOptions && menuOptions.map((option) => {
            const displayName = option.name !== undefined ? option.name : option
            const displayValue = option.value !== undefined ? option.value : option
            return (
              <MenuItem
